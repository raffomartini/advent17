...
PUZZLE = ''


'''
In [67]: reduce( (a,x) -> (x |> int) + a, '101', 0)
Out[67]: 2
'''
...
def part2(seq):
    halflen = len(seq) // 2
    halfwayseq = seq[halflen:] + seq[:halflen]
    def captcha(*_coconut_match_to_args, **_coconut_match_to_kwargs):
        _coconut_match_check = False
    ...
    return _coconut_tail_call((_coconut_partial(reduce, {0: captcha, 2: 0}, 3)), zip(seq, halfwayseq))
